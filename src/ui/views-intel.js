  ...
  V.archive = function (a) {
    ...
    set($("#archive-body"), html`
      ...
      <div class="card" style="margin-top:12px"><h3>Alpha attribution ${prov(w.month?.prov || "UNKNOWN")}</h3>
        <div class="tbl-wrap"><table class="tbl"><thead><tr><th>Fenêtre</th><th class="r">Toi</th><th class="r">BTC B&H</th><th class="r">ETH B&H</th><th class="r">Mix 50/50</th><th class="r">Alpha vs BTC</th><th class="r">avant coûts</th><th class="r">Max DD</th><th class="r">Sharpe / Sortino</th></tr></thead><tbody>${raw(winRow("week", "7 jours (perps)") + winRow("month", "30 jours (perps)") + winRow("allTime", "Depuis le début (perps)") + winRow("totalMonth", "30 jours (compte total)") + winRow("totalAllTime", "Depuis le début (compte total)"))}</tbody></table></div>
        ${isNum(al.perpsAllTimePnl) ? html`<div class="kv"><span class="k">PnL perps cumulé depuis le début (API portfolio)${prov("HISTORICAL")}</span><span class="v ${al.perpsAllTimePnl >= 0 ? "pos" : "neg"}" data-private>${fmt.usdSigned(al.perpsAllTimePnl)}</span></div>` : ""}
        ${isNum(al.capital?.deposits) ? html`<div class="kv"><span class="k">Dépôts · retraits (ledger 365j)</span><span class="v" data-private>${fmt.usd(al.capital.deposits)} · ${fmt.usd(al.capital.withdrawals)}</span></div>` : ""}
        <div class="hint">Rendement = Dietz modifié sur l'historique de valeur (séries perps de l'API, dépôts/retraits/transferts neutralisés via le ledger). « Ne rien faire » = 0%. Une hausse due uniquement au beta BTC n'est pas de l'alpha. Fills couverts : ${al.activity?.spanDays ? al.activity.spanDays.toFixed(0) + " j (" + al.fillsCount + " fills" + (al.activity.truncatedWindow ? ", fenêtre tronquée" : "") + ")" : "aucun"} ; ${al.truncatedCount} trade(s) commencé(s) avant la fenêtre exclus des statistiques.</div>
